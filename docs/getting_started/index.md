...
This package makes it easy to retrieve data from relatics and parse the resulting xml.

In order to make full use of the relatics connector a very specific webservice needs be be made, which can be found later in the getting started.

With the webservice in place a ETL run can be started which can extract a list of elements at once and dediplicate the resulting tables. Doing this will make the relatics data clean enough to be used in downstream applications or database storage.
...
!!! note
    Relatics doesn't include parts of a report if they never show up. If for example a property is added in a report part but never used in Relatics, it will not show up in the webservice and therefore not in the DataFrame.
